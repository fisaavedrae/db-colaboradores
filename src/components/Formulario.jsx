...
const Formulario = ({ onSubmit, setAlert }) => {
    ...
    const handleSubmit = (e) => {
        ...
        else if (edad < 1 || telefono < 1) {
            setAlert({
                error: true,
                msg: "La Edad y Telefono deben ser un dato numerico",
                color: "danger",
            });
            return;
        }
    ...
    return (
        <>
            ...
            <form className="d-flex flex-column justify-content-between" onSubmit={handleSubmit}>
                ...
                <input type="email" name="correo" placeholder="Email del colaborador" onChange={(e) => setCorreo(e.target.value)} value={correo} />
                <input type="number" name="edad" placeholder="Edad del colaborador" onChange={(e) => setEdad(e.target.value)} value={edad} />
                <input type="text" name="cargo" placeholder="Cargo del colaborador" onChange={(e) => setCargo(e.target.value)} value={cargo} />
                <input type="number" name="telefono" placeholder="Telefono del colaborador" onChange={(e) => setTelefono(e.target.value)} value={telefono} />
